...
### Fixed
- Fix confusion matrix CSV transposed relative to HTML report — legacy `ConfusionMatrix` class now follows sklearn convention (rows=reference, columns=predicted) (issue #53)

### Changed
- Convert `[project.optional-dependencies]` to PEP 735 `[dependency-groups]` with `include-group` references
...
### Fixed
- Replace all legacy synchronous dependency installer calls with async QgsTask-based installer to prevent macOS ARM64 SIGABRT crash (issue #48)
- Fix `StratifiedKFold` object not iterable error with scikit-learn 1.x — call `.split()` explicitly (issue #51)

### Changed
- Release workflow now gates on cross-platform tests (Ubuntu, macOS ARM64, macOS Intel) before publishing
...
### Fixed
- Defer all GUI calls from QgsTask callbacks via QTimer.singleShot to prevent macOS ARM64 SIGABRT crash (issue #48)
- Add thread-identity diagnostic logging for dependency install task debugging
